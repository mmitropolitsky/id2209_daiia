...
species Stage skills: [fipa] {
	...
	reflex abruptlyEndShow when: showDuration > showTimer and time mod 30 = 0 {
		list<Guest> guestsAttendingShow <- Guest at_distance(0);
		if (length(guestsAttendingShow) < actualBand.minAttendees) {
			write actualBand.name + " is not happy with the poor attendance (" 
					+ length(guestsAttendingShow) + " out of min " 
					+ actualBand.minAttendees +" )! SHOW IS CANCELLED!";
			do doEndShow;
		} else {
			write actualBand.name + " is VERY happy with the attendance (" 
					+ length(guestsAttendingShow) + " out of min " 
					+ actualBand.minAttendees +" )! ROCK ON!";
		}
	}
