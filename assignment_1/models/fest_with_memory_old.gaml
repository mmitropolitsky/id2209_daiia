		...
		if (infoCenter != nil and infoCenter.location = targetPoint) {
			...
			ask infoCenter {
				if (myself.thirsty = true) {
					list<DrinksStore> drinkStoresList <- getDrinksStores();
					int drinkStoreIndex <- rnd(0,length(drinkStoresList) - 1);
					write "store index: " + drinkStoreIndex;
					myself.targetPoint <- drinkStoresList[drinkStoreIndex].location;
				} else if (myself.hungry = true) {
					list<FoodStore> foodStoresList <- getFoodStores();
					int foodStoreIndex <- rnd(0, length(foodStoresList) - 1);
					write "food store index: " + foodStoreIndex;
					myself.targetPoint <- foodStoresList[foodStoreIndex].location;
				}
			}
			infoCenter <- nil;
			...
			ask drinkStore {
				...
				myself.thirst <- 0.0;
				myself.targetPoint <- {rnd(0.0, 100.0), rnd(0.0, 100.0)};
				myself.myColor <- #red;
